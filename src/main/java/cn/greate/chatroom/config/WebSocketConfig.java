package cn.greate.chatroom.config;
...
public class WebSocketConfig implements WebSocketConfigurer {
    ...
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
        ...
        registry.addHandler(handler, "/chat").addInterceptors(handshake).setAllowedOrigins("*");
        //部分 不支持websocket的访问链接,允许跨域
        registry.addHandler(handler, "/sockjs/chat").addInterceptors(handshake).setAllowedOrigins("*").withSockJS();
    }
    @Bean
